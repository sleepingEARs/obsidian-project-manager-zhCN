...
export function renderTable(ctx: TableContext): void {
  ...
  const cols: { key: SortKey | null; label: string; width?: string }[] = [
    ...
    { key: 'title', label: ctx.plugin.t('Task'), width: 'auto' },
    { key: 'status', label: ctx.plugin.t('Status'), width: '130px' },
    { key: 'priority', label: ctx.plugin.t('Priority'), width: '110px' },
    { key: 'assignees', label: ctx.plugin.t('Assignees'), width: '140px' },
    { key: 'due', label: ctx.plugin.t('Due'), width: '110px' },
    { key: 'progress', label: ctx.plugin.t('Progress'), width: '120px' },
    { key: null, label: ctx.plugin.t('Time'), width: '90px' }
  ]
  for (const col of cols) {
    ...
    if (col.key) {
      ...
      th.setAttribute('aria-label', ctx.plugin.t('Sort by {label}', { label: col.label }))
...
function fillTableBody(ctx: TableContext): void {
  ...
  const addBtn = addCell.createEl('button', { text: ctx.plugin.t('+ add task'), cls: 'pm-table-add-btn' })
